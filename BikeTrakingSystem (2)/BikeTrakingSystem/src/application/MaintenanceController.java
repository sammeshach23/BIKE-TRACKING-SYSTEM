...
public class MaintenanceController implements Initializable{
	...
	public void updateMaintenanceValue() {
	    try {
	        ...
	        Document query = new Document("user", user).append("model",selectedBikeModel);
	        Document result = bikesCollection.find(query).first();
	        double enteredAmount = Double.parseDouble(amount.getText());
	        if(result!=null) {
	        	enteredAmount = enteredAmount + result.getDouble("maintenanceAmount");
	        }
	        Document updateDoc = new Document("$set", new Document("maintenanceAmount", enteredAmount));
